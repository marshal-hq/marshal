...
public class SignatureDroppedRule implements Rule {
    ...
    public String description() {
        return "GPG signature dropped relative to prior releases";
    }
    ...
    public RuleResult evaluate(PackageContext ctx) {
        return switch (ctx.current().signatureStatus()) {
            ...
            case ABSENT -> {
                VersionMetadata previousMetadata = ctx.previous();
                boolean previousSigned = previousMetadata != null && previousMetadata.signatureStatus() == SignatureStatus.PRESENT;
